...
public class App {
    ...
    private static ArrayList<Person> inputPeopleData() {
        ...
        while (true) {
            ...
            if (age > 6 && age <= 17) {
                Child child = new Child(name, cash, age);
                people.add(child);
            } else if (age >= 18 && age <= 65) {
                System.out.print("Может купить в кредит (true/false): ");
                boolean canBuyOnCredit = Boolean.parseBoolean(scanner.nextLine());
                Adult adult = new Adult(name, cash, age, canBuyOnCredit);
                people.add(adult);
            } else if (age >= 65) {
                System.out.print("Дополнительная скидка (true/false): ");
//                boolean hasAdditionalDiscount = Boolean.parseBoolean(scanner.nextLine());
                Pensioner pensioner = new Pensioner(name, cash, age);
    ...
    private static void purchaseProducts(ArrayList<Person> people, ArrayList<Product> products) {
        for (Person person : people) {
            for (Product product : products) {
                ...
                // В этом блоке устанавливаем цену на продукт, если на него есть скидка
                ...
                //Если денег у покупателя хватает, то добавляем продукт в корзину покупателя и вычитаем стоимость из кошелька
