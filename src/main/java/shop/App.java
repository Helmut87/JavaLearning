    ...
    private static void purchaseProducts(ArrayList<Person> people, ArrayList<Product> products) {
        for (Person person : people) {
            for (Product product : products) {
                ...
                // Применение скидки, если она актуальна
                if (product.getDiscountAmount() > 0 && !product.getDiscountExpirationDate().isBefore(LocalDate.now())) {
                    priceToPay -= product.getDiscountAmount();
                }

                // Проверка на отрицательную или нулевую стоимость
                if (priceToPay <= 0) {
                    continue; // Пропускаем продукт с недопустимой стоимостью
                }

                // Проверка, может ли покупатель позволить себе купить товар
